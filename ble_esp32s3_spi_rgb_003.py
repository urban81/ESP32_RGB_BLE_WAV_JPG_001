...
ДОБАВЛЕНО: автоматическая запись входящего аудио в WAV файлы по 15 секунд.
"""
...
import sys
import time
import wave                     # <-- добавлено для аудио
...
# ========== КЛАСС ДЛЯ ЗАПИСИ АУДИО (ДОБАВЛЕН) ==========
class AudioRecorder:
    """Принимает аудиоданные по BLE и сохраняет их в WAV файлы по 15 секунд."""
    def __init__(self, chunk_duration=15):
        self.wav_file = None
        self.start_time = 0
        self.chunk_duration = chunk_duration

    def create_new_wav(self):
        """Закрывает текущий файл и создаёт новый с временной меткой."""
        if self.wav_file:
            self.wav_file.close()
        filename = f"audio_16bit_{int(time.time())}.wav"
        self.wav_file = wave.open(filename, 'wb')
        self.wav_file.setnchannels(1)          # моно
        self.wav_file.setsampwidth(2)          # 16 бит
        self.wav_file.setframerate(16000)      # 16 кГц
        self.start_time = time.time()
        print(f"\n[Audio] Начата запись: {filename}")

    def notification_handler(self, sender, data):
        """Колбэк для BLE уведомлений – сохраняет данные в текущий WAV."""
        # Если прошло 15 секунд, создаём новый файл
    ...
    def close(self):
        """Закрывает текущий WAV файл (например, при отключении)."""
        if self.wav_file:
            self.wav_file.close()
            self.wav_file = None
            print("[Audio] Запись остановлена")
...
class BLEImageSenderApp(App):
    ...
    def __init__(self, **kwargs):
        ...
        self.audio_recorder: Optional[AudioRecorder] = None   # <-- добавлено

    def build(self):
        self.title = f"BLE JPEG Sender + Audio Recorder для {TFT_WIDTH}x{TFT_HEIGHT}"
        Window.size = (800, 600)
        Window.bind(on_request_close=self.on_request_close)

        main_layout = BoxLayout(orientation='vertical', padding=10, spacing=10)

        # Статусная строка (добавлен индикатор аудио)
        status_layout = BoxLayout(size_hint=(1, 0.1), spacing=10)
        self.status_label = Label(text=self.status_text, font_size='16sp', halign='left', size_hint=(0.5,1))
        self.status_label.bind(size=self.status_label.setter('text_size'))
        self.size_label = Label(text=f"Дисплей: {TFT_WIDTH}x{TFT_HEIGHT}", font_size='12sp', halign='right', color=(0.8,0.6,0.2,1), size_hint=(0.2,1))
        self.device_label = Label(text=self.connected_device, font_size='14sp', halign='right', color=(0.2,0.8,0.2,1), size_hint=(0.2,1))
        self.audio_status_label = Label(text="🎙 Аудио: выкл", font_size='14sp', halign='right', color=(0.8,0.2,0.2,1), size_hint=(0.1,1))  # <-- добавлено
        status_layout.add_widget(self.status_label)
        status_layout.add_widget(self.size_label)
        status_layout.add_widget(self.device_label)
        status_layout.add_widget(self.audio_status_label)
        ...
        try:
            ...
            for service in self.ble_client.services:
                ...
                for char in service.characteristics:
                    print(f"   🔹 Характеристика: {char.uuid} (свойства: {char.properties})")
            ...
            # === ДОБАВЛЕНО: запуск записи аудио ===
            if 'notify' in self.tx_characteristic.properties:
                self.audio_recorder = AudioRecorder()
                self.audio_recorder.create_new_wav()  # начать первый файл
                await self.ble_client.start_notify(self.tx_characteristic, self.audio_recorder.notification_handler)
                print("🎙 Подписка на аудиоуведомления активирована")
                # Обновим GUI позже в on_device_connected
            else:
                print("⚠️ Характеристика не поддерживает уведомления, аудиозапись невозможна")
    ...
    def on_device_connected(self, address):
        ...
        self.send_button.disabled = False
        self.test_button.disabled = False
        # Обновляем индикатор аудио, если запись активна
        if self.audio_recorder:
            self.audio_status_label.text = "🎙 Аудио: запись"
            self.audio_status_label.color = (0.2,0.8,0.2,1)
    ...
    def cleanup_and_exit(self):
        ...
        # Закрываем файл аудио, если запись активна
        if self.audio_recorder:
...
if __name__ == '__main__':
    print("=" * 60)
    print("BLE JPEG Sender + Audio Recorder для ESP32 RGB Display")
    print("=" * 60)
    print(f"📐 Размер TFT дисплея: {TFT_WIDTH}x{TFT_HEIGHT}")
    print(f"⚙️ Качество JPEG: {JPEG_QUALITY}")
    print("🎙 Аудио будет сохраняться в WAV (16 бит, 16 кГц) чанками по 15 секунд")
    print("\n🚀 Запуск приложения...")
    print("=" * 60)
